...
export default function TaskPanel({board}: props) {
    ...
    const {functions: {createNewTask, changeBoardOfTask,editBoard}, states: {tasks}} = useContext(HomeContext) as any
    ...
    return (<div className={"board"} onDropCapture={onDrop} onDragOver={onDragOver} onDragLeave={onDragLeave}
                 style={{
                     transition: "0.3s",
                     flex: "0 0 auto",
                     padding: "10px",
        ...
        <div draggable style={{width: "100%", display: "flex", alignItems: "center", background: "#ffc1bd"}}>
            <Text onChanged={(change)=>{editData.title=change;editBoard(id,editData)}} editable size={1.2} style={{margin: "7px"}}>{title}</Text>
        </div>
        ...
        <div style={{width: "100%", marginTop: "10px", overflowY: "auto"}}>
            <div style={{
                ...
                <Button onClick={() => createNewTask(id)} style={{width: "100%", padding: "10px"}} variant="text">Add
                    Task</Button>
            </div>
            {boardTasks.map((data: TaskItemProps, i: number) => <TaskItem key={i} {...data} boardId={id}/>)}
        </div>
    </div>);
}
